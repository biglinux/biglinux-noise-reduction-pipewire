...
TITLE = 'fix(ui): serialize microphone commands and follow external device changes'
if not done(TITLE):
    write('src/ui/widgets/source_picker.rs', r'''//! Microphone controls with one serialized, coalescing worker lane.
//!
...
''')
    commit(TITLE, ['src/ui/widgets/source_picker.rs'])
