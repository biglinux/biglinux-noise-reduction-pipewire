...
TITLE = 'fix(applet): report real readiness and back off failed watchers'
if not done(TITLE):
    path = 'src/bin/cli.rs'
    replace(path, 'enum Key {\n    Mic,', 'enum Key {\n    Mic,\n    OutputMaster,')
    replace(path, '            "mic" => Self::Mic,', '            "mic" => Self::Mic,\n            "output" => Self::OutputMaster,')
    replace(path, '            Key::Mic => match on_or_off(&value) {', '            Key::Mic => match on_or_off(&value) {') if False else None
    replace(path, '        Key::Mic => match on_or_off(&value) {', '''        Key::OutputMaster => match on_or_off(&value) {
            Some(enabled) => settings.output_filter.enabled = enabled,
            None => return not_a_switch(&name, &value),
        },
        Key::Mic => match on_or_off(&value) {''')
    text = Path(path).read_text()
    start = text.index('fn print_status()'); end = text.index('\n}', start) + 2
    text = text[:start] + '''fn print_status() -> ExitCode {
    ...
    text = text.replace('NAMES: &\'static str = "mic, mic-intensity', 'NAMES: &\'static str = "mic, output, mic-intensity')
    Path(path).write_text(text)
    write('usr/share/plasma/plasmoids/br.com.biglinux.micnoise/contents/ui/main.qml', r'''/* SPDX-License-Identifier: GPL-2.0-or-later */
import QtQuick
...
''')
    commit(TITLE, [path, 'usr/share/plasma/plasmoids/br.com.biglinux.micnoise/contents/ui/main.qml'])
